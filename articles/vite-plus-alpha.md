...
- `oxlint`: type-aware な場合はまだ厳しい
  ...
  - type-aware なルールについては TypeScript 7.0+ が前提とされているので、TS 6 以下 (つまり現行のほとんど) のプロジェクトにとっては移行コストがまだ大きい
    - 一応 `baseUrl` などの一部のレガシー設定を使用しなければ動きはするっぽい。このあたり誰か整理してくれないかな。
...
`vite.config.ts` だと `cache` 有無やトラックする環境変数なども制御できるようですので、ロックインを許容するのであればこっちがファーストチョイスになるかもしれません。
...
- まだ Alpha なので当然だが、商用で使えるほど安定ではない (特に oxlint や tsgolint)
  - とはいえ PoC や社内システムくらいなら `vp lint` 込みでも十分使える感もあります

自分はそこまでツール乱立が気にならないタイプなのですが、実際ツールチェーンが公式に統一されている Go や Deno の開発体験は凄く良いので、折をみて貢献していければと思います！Alpha で盛り上がり過ぎ感は否めないですが、[Rome](https://github.com/rome/tools) の二の舞にならないよう、持続可能性のための十分な支援が集まってほしいですね！