...
出力中の一瞬の間に初見のフレーバーテキストについて考察するのはなかなか難しいですが、保存さえしておけば、こんな風に後から見返して楽しむことができます。

## 免責ing…

こんなふとした楽しみを与えてくれる `cc-flavors` ですが、以下の制約があります
...
以上、Claude Code のフレーバーテキスト (e.g. `✢ Sparkling…`) コレクションツール `cc-flavors` の紹介でした。

非tmuxユーザやptyに詳しい方など、各位コントリビュートお待ちしております！
